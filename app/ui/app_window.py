...
FIX: self.root = ctk.CTk() BEFORE self.project = ctk.StringVar()
"""
...
class AppWindow:
    def __init__(self, user: dict):
        ...
        self.root = ctk.CTk()
        self.root.title(
            f"TeamCyberOps V5  //  {user['username'].upper()}")
        self.root.configure(fg_color=C["bg_app"])
        self.root.minsize(1280, 720)

        # StringVar AFTER root
    ...
    def _build_topbar(self):
        bar = ctk.CTkFrame(self.root, height=44,
                           fg_color=C["bg_panel"], corner_radius=0)
        bar.pack(fill="x", side="top")
        bar.pack_propagate(False)
        ctk.CTkFrame(bar, height=1, fg_color=C["border_mid"],
                     corner_radius=0).place(relx=0, rely=1,
                                            relwidth=1, anchor="sw")

        # Logo thumbnail in topbar
        ...
        ctk.CTkLabel(wm, text="TeamCyberOps",
                     text_color=C["accent"],
                     font=F(13, bold=True)).pack(side="left")
        ctk.CTkLabel(wm, text="  v5",
        ...
        role_c = C["accent"] if self.user.get("role") == "admin" \
                 else C["green"]
        ...
        for label, cmd, color in [
            ...
            ("[LOGOUT]",   self.root.destroy,                  C["red"]),
        ]:
            NeonButton(bar, text=label, command=cmd,
                       color=color, small=True).pack(
                side="right", padx=4, pady=9)

    def _build_statusbar(self):
        bar = ctk.CTkFrame(self.root, height=22,
                           fg_color=C["bg_panel"], corner_radius=0)
        ...
        self._status_lbl = ctk.CTkLabel(
            ...
            text_color=C["accent"],
            font=F(8, mono=True), anchor="w")
        self._status_lbl.pack(side="left", padx=10)

        import sys
        ctk.CTkLabel(bar,
                     text=f"Python {sys.version[:6]}  |  SQLite  |  Gemini AI  |  TeamCyberOps v5",
                     text_color=C["text_dim"],
                     font=F(8)).pack(side="right", padx=10)
    ...
    def _build_sidebar(self, parent):
        sb = ctk.CTkFrame(parent, width=204,
                          fg_color=C["bg_sidebar"], corner_radius=0)
        sb.pack(side="left", fill="y")
        sb.pack_propagate(False)
        ctk.CTkFrame(sb, width=1, fg_color=C["border_mid"],
        ...
        for i, (cat, name, _) in enumerate(TABS):
            ...
            for w in [row, name_lbl, ind]:
                w.bind("<Button-1>",
                       lambda e, idx=i: self._switch_to(idx))
                w.bind("<Enter>", _enter)
                w.bind("<Leave>", _leave)

    def _build_content(self, parent):
        self._content = ctk.CTkFrame(parent,
                                      fg_color=C["bg_app"],
                                      corner_radius=0)
    ...
    def _switch_to(self, idx: int):
        if 0 <= self._cur_idx < len(self._sb_rows):
            ...
            nl.configure(text_color=C["text_muted"], font=F(10))
        ...
        if 0 <= idx < len(self._sb_rows):
            ...
            nl.configure(text_color=cc, font=F(10, bold=True))
            ind.configure(fg_color=cc, width=2)
    ...
    def _filter_sidebar(self, *_):
        ...
        for i, (row, nl, cc, ind) in enumerate(self._sb_rows):
            ...
            if show: row.pack(fill="x", padx=4, pady=1)
    ...
    def _err_tab(self, frame, name, ex):
        ...
        ctk.CTkLabel(sc,
                     text=f"[ERR] Failed to load: {name}",
                     text_color=C["red"],
                     font=F(13, bold=True, mono=True)
                     ).pack(anchor="w")
        ctk.CTkLabel(sc,
    ...
    def _new_project(self):
        ...
        FilledButton(win, text=">> CREATE",
                     command=_ok, color=C["green"],
                     height=32).pack(fill="x", padx=14, pady=8)
    ...
    def _fetch_ip(self):
        try:
            ...
            self.root.after(0, lambda: self._ip_lbl.configure(
                text=f"IP:{ip}", text_color=C["text_dim"]))
        except Exception:
            pass
