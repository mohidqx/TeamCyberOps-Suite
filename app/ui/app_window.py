...
class AppWindow:
    ...
    def _on_close(self):
        """Clean shutdown: cancel pending after() callbacks before destroying."""
        try:
            # Cancel all pending after() events to prevent
            # 'invalid command name' errors from CTk internal timers
    ...
    def _build_statusbar(self):
        ...
        bar.pack_propagate(False)
        # Red top border on statusbar (C2 style)
    ...
    def _build_sidebar(self, parent):
        ...
        # Right border: accent-colored 1px vertical line
    ...
    def _switch_to(self, idx: int):
        if 0 <= self._cur_idx < len(self._sb_rows):
            ...
            nl.configure(text_color=C["text_muted"], font=F(11))
        ...
        if 0 <= idx < len(self._sb_rows):
            ...
            nl.configure(text_color=cc, font=F(11, bold=True))
    ...
    def _fetch_ip(self):
        try:
            ...
            # Guard: only update if root still exists
            def _upd():
                try:
