    ...
    def _load_proj_detail(self, name):
        ...
        if not findings:
            ...
            for f in findings:
                ...
                ctk.CTkLabel(row, text=f" {sev[:4]} ", text_color=color,
                             fg_color=C["bg_hover"], font=F(9, bold=True, mono=True), width=54).pack(side="left", padx=(8,6), pady=6)
        ...
        for sev, cnt, color in [
            ...
            ctk.CTkLabel(bar_row, text=f" {sev} ", text_color=color,
                         fg_color=C["bg_hover"], font=F(10, bold=True, mono=True), width=80).pack(side="left")
