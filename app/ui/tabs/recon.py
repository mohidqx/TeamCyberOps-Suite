...
class ReconMixin:
    ...
    def _mk_recon_tab(self, frame, title, icon, desc, run_fn,
        ...
        """
        SPLIT LAYOUT:
          TOP  ── controls (fixed height, does NOT stretch)
          ─── accent separator line ───
          BOT  ── terminal fills ALL remaining vertical space
        """
        frame.configure()
        c = btn_color or C["accent"]

        # ── TOP: Controls (no scrollable frame — fixed height) ──────────
        ctrl = ctk.CTkFrame(frame, fg_color="transparent", corner_radius=0)
        ctrl.pack(fill="x", side="top", padx=12, pady=(8, 0))

        Section(ctrl, title, icon, c).pack(fill="x", pady=(0, 8))

        info = Card(ctrl, accent=c)
        info.pack(fill="x", pady=(0, 8))
        ctk.CTkLabel(info, text=desc,
                     font=F(10), justify="left", wraplength=900,
        ...
        for label, key, default, width in (fields or []):
            ...
            ctk.CTkLabel(row, text=label,
                         font=F(11, mono=True), width=140,
                         anchor="e").pack(side="left", padx=(0, 10))
            v = ctk.StringVar(value=default)
            GlowEntry(row, textvariable=v,
                      width=width or 400, height=32).pack(side="left")
            if key == "target":
                NeonButton(row, text="← Project", small=True,
                           color=C["text_dim"],
        ...
        def _run():
            ...
            threading.Thread(
                target=lambda: run_fn(vals, term.log, stop_flag, self),
                daemon=True).start()
        ...
        FilledButton(btn_row, text=btn_label, command=_run,
                     color=c).pack(side="left", ipady=4)
    ...
    def _tab_active_recon(self, frame):
        ...
        # Controls top
        ctrl = ctk.CTkFrame(frame, fg_color="transparent", corner_radius=0)
        ctrl.pack(fill="x", side="top", padx=20, pady=(12, 0))
        Section(ctrl, "ACTIVE RECON", "⚡", C["yellow"]).pack(fill="x", pady=(0, 8))
    ...
    def _build_simple_recon_subtab(self, parent, title, run_fn, fields, color):
        """Sub-tab: inputs on top, terminal fills the rest."""
        vars_map = {}
        ...
        for label, key, default, width in fields:
            ...
            ctk.CTkLabel(row, text=label+":",
                         font=F(11, mono=True), width=120, anchor="e").pack(side="left", padx=(0,8))
            v = ctk.StringVar(value=default)
    ...
    def _tab_origin_hunter(self, frame):
        ...
        ctrl = ctk.CTkFrame(frame, fg_color="transparent", corner_radius=0)
        ctrl.pack(fill="x", side="top", padx=20, pady=(12, 0))
        Section(ctrl, "ORIGIN HUNTER — Find Real IP Behind CDN/WAF", "🛡", C["orange"]).pack(fill="x", pady=(0, 8))
    ...
    def _tab_auto_recon(self, frame):
        """Split layout: controls top, terminal bottom half."""
        frame.configure()
        ...
        Section(ctrl, "AUTO-RECON — Full 5-Phase Pipeline", "🤖", C["accent"]).pack(fill="x", pady=(0, 8))

        info = Card(ctrl, accent=C["accent"])
        info.pack(fill="x", pady=(0, 8))
        ...
        ), font=F(10), justify="left", anchor="w").pack(anchor="w", padx=14, pady=8)

        # Target input
        row = ctk.CTkFrame(ctrl, fg_color="transparent")
        row.pack(fill="x", pady=4)
        ctk.CTkLabel(row, text="Target Domain:",
                     font=F(11, mono=True), width=140, anchor="e").pack(side="left", padx=(0,10))
        ...
        # Phase checkboxes
        self._ar_phases = {}
        phases_row = ctk.CTkFrame(ctrl, fg_color="transparent")
        phases_row.pack(fill="x", pady=6)
        for phase, default, color in [
            ...
            ctk.CTkCheckBox(phases_row, text=phase, variable=v, fg_color=color,
                            hover_color=C["bg_hover"], font=F(11)).pack(side="left", padx=10)
        ...
        for ph, color in [("Passive","cyan"),("HTTP","yellow"),
                          ("URLs","green"),("Vulns","red"),("OSINT","purple")]:
            lbl = ctk.CTkLabel(prog_row, text=f"○ {ph}", font=F(10, mono=True))
        ...
        def _go():
            try:
                ...
                self.root.after(0, lambda: self.set_status(
                    f"Auto-recon done: {domain}", C["green"]))

            except Exception as _e:
                import traceback as _tb
                self._ar_term.log(f"[-] Error: {_e}", "err")
    ...
    def _tab_dorks(self, frame):
        ...
        ctrl = ctk.CTkFrame(frame, fg_color="transparent", corner_radius=0)
        ctrl.pack(fill="x", side="top", padx=20, pady=(12, 0))
        Section(ctrl, "DORKS — 270+ Queries (Google · Shodan · Censys · GitHub)", "🔎",
                C["yellow"]).pack(fill="x", pady=(0, 8))

        info = Card(ctrl, accent=C["yellow"])
        info.pack(fill="x", pady=(0, 8))
        ctk.CTkLabel(info, text=(
        ...
        NeonButton(row, text="← Project", small=True, color=C["text_dim"],
                   command=lambda v=None: self._dork_target.set(
                       self.project.get() or "")).pack(side="left", padx=6)
        ...
        def _gen_dorks():
            ...
            def _go():
                ...
                for name, dorks in [("Google",g),("Shodan",s),
                                    ("Censys",c),("GitHub",gh)]:
                ...
                self.root.after(0, lambda: self.set_status(
                    f"Dorks saved: {len(all_dorks)} total", C["green"]))
            threading.Thread(target=_go, daemon=True).start()
