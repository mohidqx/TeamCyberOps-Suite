...
# ══════════════════════════════════════════════════════════════════════
#  BUG BOUNTY METHODOLOGY ONELINERS (from user-provided reference)
# ══════════════════════════════════════════════════════════════════════
...
SUBDOMAIN_ENUM = {
    "Passive Discovery": [
        ...
        'curl -s "https://crt.sh/?q=%.{target}&output=json" | jq -r \'.[].name_value\' | grep -v \'*\' | sort -u > crtsh.txt',
        'curl -s "http://web.archive.org/cdx/search/cdx?url=*.{target}/*&output=text&fl=original&collapse=urlkey" | sort | sed -e \'s_https*://__\' -e \'s/\\/.*//\' | sort -u > wayback_subs.txt',
    ],
    "Active Enumeration": [
        'amass enum -active -d {target} | cut -d\']\' -f 2 | awk \'{print $1}\' | sort -u > amass_active.txt',
        'ffuf -u "https://FUZZ.{target}" -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt -mc 200,301,302',
        'subfinder -d {target} | alterx | dnsx',
        'echo {target} | alterx -enrich | dnsx',
        'echo {target} | alterx -pp word=/usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt | dnsx',
    ],
    "Merge & Resolve": [
        'cat *.txt | sort -u > all_subs.txt',
        'cat all_subs.txt | dnsx -silent -resp-only > resolved.txt',
        'cat all_subs.txt | httpx-toolkit -ports 80,443,8080,8000,8888 -threads 200 > alive.txt',
        'cat all_subs.txt | httpx-toolkit -sc -title -server -td -threads 200 | tee httpx_results.txt',
    ],
}

URL_COLLECTION = {
    "Crawling": [
        'katana -u {target} -d 2 -o urls.txt',
        'cat alive.txt | hakrawler -u > urls_crawled.txt',
        'cat alive.txt | gau | sort -u > gau_urls.txt',
        'echo {target} | gau --mc 200 | urldedupe > urls200.txt',
        'urlfinder -d {target} | sort -u > urlfinder.txt',
    ],
    "URL Filtering": [
        'cat allurls.txt | grep \'=\' | urldedupe | tee params.txt',
        'cat allurls.txt | grep -E \'\\.php|\\.asp|\\.aspx|\\.jspx|\\.jsp\' | grep \'=\' | sort > dynamic.txt',
        'cat allurls.txt | grep -E \'\\.(xls|xml|xlsx|json|pdf|sql|doc|zip|bak|7z|log|env|config|yaml|key|pem)$\' > sensitive.txt',
        'cat allurls.txt | gf sqli > sqli_urls.txt',
        'cat allurls.txt | gf redirect | uro | sort -u > redirect_params.txt',
        'cat allurls.txt | gf lfi | uro | sed \'s/=.*/=/\' > lfi_params.txt',
        'cat urls.txt | uro | sort -u > deduped.txt',
...
DIRECTORY_BRUTEFORCE = {
    "Dirsearch": [
        'dirsearch -u https://{target} --full-url --deep-recursive -r',
        'dirsearch -u https://{target} -e php,html,js,txt,bak,zip,old,conf,log,asp,aspx,jsp,sql,json,xml,yml,py --random-agent --recursive -R 3 -t 20 --exclude-status=404',
    ],
    "FFUF": [
        'ffuf -w /usr/share/seclists/Discovery/Web-Content/directory-list-2.3-big.txt -u https://{target}/FUZZ -fc 400,401,403,404,429,500 -recursion -recursion-depth 2 -e .html,.php,.txt,.js,.zip,.bak,.log,.json,.config,.env -ac -c -t 10',
        'ffuf -u "https://FUZZ.{target}" -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt -mc 200,301,302',
    ],
    "Parameter Discovery": [
        'arjun -u https://{target}/endpoint.php -oT arjun.txt -t 10 --rate-limit 10 --passive -m GET,POST',
        'arjun -u https://{target}/api -m GET,POST -w /usr/share/seclists/Discovery/Web-Content/burp-parameter-names.txt -t 10',
    ],
}

VULN_SCANNING = {
    "Nuclei": [
        'nuclei -u https://{target} -bs 50 -c 30',
        'nuclei -l alive.txt -s critical,high -bs 50 -c 30',
        'nuclei -l alive.txt -tags cve2024 -severity critical',
        'cat alive.txt | httpx-toolkit -silent | nuclei -t nuclei-templates/vulnerabilities/cors/ -o cors_results.txt',
    ...
    "Open Redirect": [
        ...
        'subfinder -d {target} -all | httpx-toolkit -silent | gau | gf redirect | uro | qsreplace "https://evil.com" | httpx-toolkit -silent -fr -mr "evil.com"',
    ],
    "LFI Testing": [
        'echo "https://{target}/" | gau | gf lfi | uro | sed \'s/=.*/=/\' | qsreplace "FUZZ" | sort -u | xargs -I{} ffuf -u {} -w /usr/share/seclists/Fuzzing/LFI/LFI-Jhaddix.txt -c -mr "root:(x|\\*|\\$[^\\:]*):0:0:" -v',
    ...
    "CORS Testing": [
        'curl -H "Origin: http://evil.com" -I https://{target}/api/ | grep -i "access-control"',
...
RECON_METHODOLOGY = {
    "ASN Discovery": [
        ...
        'shodan search "org:{target}" --limit 1000 | jq ".ip_str"',
        'curl -s "https://otx.alienvault.com/api/v1/indicators/hostname/{target}/url_list?limit=500" | jq -r \'.url_list[]?.result?.urlworker?.ip // empty\'',
        'curl -s "https://urlscan.io/api/v1/search/?q=domain:{target}&size=10000" | jq -r \'.results[]?.page?.ip // empty\'',
    ],
    "Screenshots": [
    ...
    "Git Exposure": [
        'cat domains.txt | httpx-toolkit -sc -server -path "/.git/" -mc 200 -ms "Index of"',
    ],
    "Sensitive Files": [
        'cat allurls.txt | grep -E "\\.(xls|xml|xlsx|json|pdf|sql|doc|docx|pptx|txt|zip|tar\\.gz|bak|7z|log|env|config|csv|yaml|key|pem|cert)$"',
    ],
    "WordPress": [
        'wpscan --url https://{target} --disable-tls-checks --api-token YOUR_TOKEN -e at -e ap -e u --plugins-detection aggressive',
    ],
    "Subdomain Takeover": [
...
# ── Append new categories to ALL_ONELINER_CATEGORIES ────────────────
ALL_ONELINER_CATEGORIES.update({
    "📡 Subdomain Enumeration":     SUBDOMAIN_ENUM,
    "🔗 URL Collection & Filtering": URL_COLLECTION,
    "📂 Directory Bruteforcing":    DIRECTORY_BRUTEFORCE,
    "🔎 Vuln Scanning Chains":      VULN_SCANNING,
    "🗺 Full Recon Methodology":    RECON_METHODOLOGY,
})
