...
def check_dependencies() -> dict:
    """Return dict of {tool: bool} for required tools."""
    import shutil
    tools = ['curl', 'dig', 'nmap', 'masscan', 'whatweb', 'wafw00f', 'shodan']
    return {t: shutil.which(t) is not None for t in tools}


def build_cmd_stealth_waf(target: str) -> list:
    """Return command list for stealth WAF detection."""
    clean = target.replace('https://', '').replace('http://', '').split('/')[0]
    return ['bash', '-c',
        f'wafw00f https://{clean} 2>/dev/null || '
        f'curl -sk -H "X-Forwarded-For: 127.0.0.1" https://{clean}/ -I | grep -i -E "server:|x-powered|via:|x-cache"'
    ]


def build_cmd_origin_mapper(target: str) -> list:
    """Return command list for origin IP mapping."""
    clean = target.replace('https://', '').replace('http://', '').split('/')[0]
    return ['bash', '-c',
        f'echo "=== DNS ===" && dig +short A {clean} && '
        f'echo "=== Historical (SecurityTrails) ===" && '
        f'curl -sk "https://securitytrails.com/domain/{clean}/history/a" | grep -oE "([0-9]{{1,3}}\\.?){{4}}" | sort -u && '
        f'echo "=== Shodan ===" && shodan host $(dig +short A {clean} | head -1) 2>/dev/null | head -20'
    ]


def build_cmd_full_origin_hunt(target: str) -> tuple:
    """Return (waf_cmd, map_cmd) tuple for full origin hunt."""
    return build_cmd_stealth_waf(target), build_cmd_origin_mapper(target)
