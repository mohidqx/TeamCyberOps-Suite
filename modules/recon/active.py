...
def build_cmd_masscan(target: str, ports: str = "0-65535", rate: int = 10000) -> list:
    return ['bash', '-c',
        f'masscan {target} -p{ports} --rate={rate} 2>&1 || '
        f'echo "[!] masscan not installed: sudo apt install masscan"']

def build_cmd_dirsearch(target: str, wordlist: str = "", extensions: str = "php,html,js,txt,json,xml,bak,zip") -> list:
    cmd = f'dirsearch -u {target} -e {extensions} --random-agent -t 20 --exclude-status=404'
    if wordlist:
        cmd += f' -w {wordlist}'
    cmd += ' 2>&1'
    return ['bash', '-c', cmd +
        ' || echo "[!] dirsearch not installed: pip3 install dirsearch"']
