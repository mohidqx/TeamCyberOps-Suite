...
def build_cmd_nuclei_cves(target: str, year: str = "2024") -> list:
    return ['bash', '-c',
        f'nuclei -u {target} -tags cve{year} -severity critical,high -bs 30 -c 20 2>&1']

def build_cmd_dalfox(target: str, params: str = "") -> list:
    t = target.rstrip('/')
    return ['bash', '-c',
        f'dalfox url "{t}" --skip-bav --only-poc r --silence 2>&1 || '
        f'echo "[!] dalfox not installed: go install github.com/hahwul/dalfox/v2@latest"']

def build_cmd_sqlmap(target: str, data: str = "", level: int = 1, risk: int = 1,
                     tamper: str = "", random_agent: bool = True) -> list:
    cmd = f'sqlmap -u "{target}" --batch --level={level} --risk={risk}'
    if data:       cmd += f' --data="{data}"'
    if tamper:     cmd += f' --tamper={tamper}'
    if random_agent: cmd += ' --random-agent'
    cmd += ' 2>&1'
    return ['bash', '-c', cmd]

def build_cmd_nikto(target: str, port: int = None) -> list:
    t = target.rstrip('/')
    cmd = f'nikto -h {t}'
